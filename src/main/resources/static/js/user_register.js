 ...
 * 描述: 用户注册表单校验器注册
 */
function user_register_validator() {
    //重置按钮的点击事件监听函数注册
            ...
            "user-register-form-code-input": {
                ...
                validators: {
                    ...
                    remote: {
                        delay: 2000,
                        message: "*验证码不正确",
                        url: requestmap.user_register_code,
                        type: "post",
    ...
    $("#user-register-form-code-button").on("click", function () {
        //获取验证实例和邮箱输入框实例
        let form_instance = $("#user-register-form").data("bootstrapValidator");
        let uemail_input = $("#user-register-form-email-input");
        //手动验证邮箱
        form_instance.validateField(uemail_input);
        //判断邮箱是否正确
        if (form_instance.isValidField(uemail_input)) {
            //修改验证码发送状态
            $(this).button("loading");
